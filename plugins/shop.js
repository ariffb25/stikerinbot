            ...
            case 'buy':
                switch (_type) {
                    case 'potion':
                        ...
                        break
case 'pancing':
if (global.db.data.users[m.sender].pancing == 1 ) throw 'Lu dah punya'
                        if (global.db.data.users[m.sender].money >= pancing * count) {
                            global.db.data.users[m.sender].pancing += count * 1
                            global.db.data.users[m.sender].money -= pancing * count
                            conn.reply(m.chat, `Succes membeli ${count} kail pancing🎣 dengan harga ${pancing * count} money`, m)
